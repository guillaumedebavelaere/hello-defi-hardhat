...
!developmentChains.includes(network.name)
    ...
    : describe("FeesCollector test", async () => {
          const deployFeesCollectorFixture = async () => {
              ...
              const feesCollector: FeesCollector = await ethers.getContract(
                  "FeesCollector"
              );
          ...
          it("should deploy with no errors", async function () {
              const { feesCollector } = await loadFixture(
                  deployFeesCollectorFixture
              );
          ...
          describe("withdraw test", async () => {
              ...
              it("should revert if the user is not the owner", async () => {
                  await expect(
                      feesCollector
                          ...
                          .withdraw(erc20.address, utils.parseEther("1"))
              ...
              it("should transfer the asset to the owner wallet", async () => {
                  const ownerBalanceBeforeWithdraw = await erc20.balanceOf(
                      owner.address
                  );
                  expect(ownerBalanceBeforeWithdraw).to.be.equal(
                      utils.parseEther("0")
                  );
                  const feesCollectorBalanceBeforeWithDraw =
                      await erc20.balanceOf(feesCollector.address);
                  expect(feesCollectorBalanceBeforeWithDraw).to.be.equal(
                      utils.parseEther("100000")
                  );
                  ...
                  const ownerBalanceAfterWithdraw = await erc20.balanceOf(
                      owner.address
                  );
                  expect(ownerBalanceAfterWithdraw).to.equal(
                      utils.parseEther("100000")
                  );
                  const feesCollectorBalanceAfterWithdraw =
                      await erc20.balanceOf(feesCollector.address);
                  expect(feesCollectorBalanceAfterWithdraw).to.equal(
                      utils.parseEther("0")
                  );
              });
          });
