...
!developmentChains.includes(network.name)
    ...
    : describe("HelloDefiAAVE2 contract test", async () => {
          const deployHelloDefiAAVE2Fixture = async () => {
              ...
              const feesCollectorhelloDefiAAVE2: FeesCollector =
                  await ethers.getContract("FeesCollector");

              await deployments.deploy("HelloDefiAAVE2", {
                  from: owner.address,
              });
              const helloDefiAAVE2: HelloDefiAAVE2 = await ethers.getContract(
                  "HelloDefiAAVE2"
              );
          ...
          describe("deposit", async () => {
              ...
              context("when the smart contract has allowance", async () => {
                  ...
                  it("should transfer (erc20 - the fees) from the user to the smart contract", async () => {
                      ...
                      const erc20Balance =
                          await helloDefiAAVE2.depositedBalance(erc20.address, {
                              from: owner.address,
                          });
                      expect(erc20Balance).to.equal(utils.parseEther("49"));

                      const erc20AvgCost = await helloDefiAAVE2.assetAvgCost(
                          erc20.address,
                          { from: owner.address }
                      );
                  ...
                  it("should emit a Deposit event", async () => {
                      expect(
                          ...
                          .to.emit(helloDefiAAVE2, "Deposit")
          ...
          describe("withdraw", async () => {
              ...
              beforeEach(async () => {
                  ...
                  expect(
                      await erc20.balanceOf(helloDefiAAVE2.address)
                  ).to.equal(utils.parseEther("49"));
                  expect(
                      await helloDefiAAVE2.depositedBalance(erc20.address)
                  ).to.equal(utils.parseEther("49"));
                  expect(
                      await helloDefiAAVE2.assetAvgCost(erc20.address)
              ...
              it("should emit a Withdraw event", async () => {
                  expect(
                      ...
                      .to.emit(helloDefiAAVE2, "Withdraw")
                      .withArgs(erc20.address, utils.parseEther("49"));
              });
              ...
              context(
                  ...
                  async () => {
                      ...
                      it("should transfer the smart contract amount balance - the performance fees to the user", async () => {
                          ...
                          expect(
                              await helloDefiAAVE2.depositedBalance(
                                  erc20.address
                              )
                          ).to.equal(utils.parseEther("0"));
                      });
